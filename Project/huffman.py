...
class HuffmanCoding:
    ...
    def decompress(self, input_path):
        filename, file_extension = os.path.splitext(self.path)
        output_path = filename + "_deccrypted.txt"
...
def main():
    ...
    if user_choice == '1':
        ...
        print("Your encryption key is saved at key.txt")
        with open('key.txt', 'w') as f:
            for k in h.codes:
                ...
                f.write(h.codes[k])
                f.write('\n')
        print('Encryption Successful! Encrypted file is saved at ', output_path)
        ...
        with open(key_file, 'r') as f:
            for line in f:
                values = line.strip().split(',')  # split using ',' as delimiter
                if len(values) == 2:
                    key_dict[values[0]] = values[1]
        h.codes = key_dict
        
        output_path = h.decompress(file_path)
        print('Decryption Successful! Decrypted file is saved at:', output_path)
